                        ...
                        if (document.getElementById("address-list-row")) {
                            ...
                            <div class="card-wrap border rounded mb-4">
                                ...
                                <div class="card-wrap-body px-3 py-3">
                                    <p class="mb-0"><span class="fw-bold">Пълно Име: </span>${
                                        parent.querySelector(
                                            '[name="full_name"]'
                                        ).value
                                    }</p>
                                    <p class="mb-0"><span class="fw-bold">Имейл: </span>${
                                        parent.querySelector('[name="email"]')
                                            .value
                                    }</p>
                                    <p class="mb-0"><span class="fw-bold">Телефон: </span>${
                                        parent.querySelector('[name="mobile"]')
                                            .value
                                    }</p>
                                    <p class="mb-0"><span class="fw-bold">Метод на доставка: </span>${
                                        parent.querySelector(
                                            '[name="delivery_method"]'
                                        ).selectedOptions[0].text
                                    }</p>
                                    <p class="mb-0"><span class="fw-bold">Град: </span>${
                                        parent.querySelector('[name="city"]')
                                            .value
                                    }</p>
                                    <p class="mb-3"><span class="fw-bold">Адрес: </span>${
                                        parent.querySelector('[name="address"]')
                                            .value
                                    }</p>
                                    <div class="mt-3">
                                        <input id="address${
                                ...
                                data.address_id
                            }" class="radio-custom" name="address" type="radio" checked>
                                        <label for="address${
                                            data.address_id
                                        }" class="radio-custom-label">Избери адрес</label>
                                    </div>
                                </div>
                            </div>
                        `;
                            document
                                .getElementById("address-list-row")
                                .appendChild(newCard);

                            parent.style.display = "none";
                        } else {
                            alert(
                                "Адресът е запазен. Можете да продължите с поръчката!"
                            );
                        }
                    } else {
